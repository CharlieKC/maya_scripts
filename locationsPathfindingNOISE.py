...
class GridWithWeights(SquareGrid):
    ...
    def cost(self, from_node, to_node):
        return self.weights.get(to_node, 1) #TODO: add cost for a verticle move
...
while False in cubesInFinalPos.values():
    ...
    for cube in cmds.ls('myCube*', tr=True):
        diagram4.walls.append(tuple(getCubePos(cube)[::2]))
    #TODO: add all current cubes to walls
    for cube in cmds.ls('myCube*', tr=True):
        if not cubesInFinalPos[cube]:
            currentPos = tuple(getCubePos(cube)[::2])
            try:
                diagram4.walls.remove(currentPos)
                if currentPos == cubesGoalPos[cube]:
                    cubesInFinalPos[cube] = True
                else:
                    came_from, cost_so_far = a_star_search(diagram4, currentPos, cubesGoalPos[cube])
                    path = reconstruct_path(came_from, currentPos, cubesGoalPos[cube])

                    while path != []:
                        cubePos = tuple(getCubePos(cube)[::2])
                        nextMove = path.pop(0)
                        #print("currentPos:",cubePos, " nextMove", nextMove, cubePos==nextMove)
                        if nextMove != cubePos:
                            diagram4.walls.append(currentPos)
                            diagram4.walls.append(nextMove)
                            xMove = nextMove[0] - cubePos[0]
                            zMove = nextMove[1] - cubePos[1]
                            rollCube([xMove,0,zMove],getLastKeyframe(cube)+1,10,cube)
                            break #once cube has moved once

            except:
                print("%s obstructed" % cube)
                cubesFailedMoves[cube] += 1
                diagram4.walls.append(currentPos)
                rollCube([0,0,0],getLastKeyframe(cube)+1,10,cube)
                print("%d times" % cubesFailedMoves[cube])
            ...
            diagram4.walls.append(currentPos)
            rollCube([0,0,0],getLastKeyframe(cube)+1,10,cube)
        if cubesFailedMoves[cube] > 15: #stop the cube from trying to move if it can't reach its destination
            cubesInFinalPos[cube] = True
            print("%s cube failed to reach destination" % cube, cubesGoalPos[cube])
    time +=12
