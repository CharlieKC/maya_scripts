...
while False in cubesInFinalPos.values():
    ...
    for cube in cmds.ls('myCube*', tr=True):
        ...
        if cubesFailedMoves[cube] > 5: #stop the cube from trying to move if it can't reach its destination
            cubesInFinalPos[cube] = True
            print("%s cube failed to reach destination" % cube)
    time +=12
